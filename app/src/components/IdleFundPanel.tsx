...
export default function IdleFundPanel({
  ...
  return (
    <div className="space-y-4">
      {/* Summary Row */}
      {!loading && vaultData?.runtime && (
        <div className={`px-4 py-2 rounded-xl border text-xs font-semibold ${vaultData.runtime.activeMode === 'live' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
          DeFi runtime: {vaultData.runtime.activeProvider.toUpperCase()} ({vaultData.runtime.activeMode})
          {vaultData.runtime.emergencyStop ? ' · Emergency stop active' : ''}
        </div>
      )}
