...
function formatRegionalLiquidity(amount: number) {
  if (amount >= 10000000) {
  ...
  const handleVerifyScan = async () => {
    ...
    try {
      ...
      if (txHash.startsWith('{')) {
        ...
        txHash = parsed.txHash || parsed.transactionId || txHash;
      }
  ...
  if (activeSection === 'scanner') {
    return (
      <div className="p-6 lg:p-10 max-w-5xl mx-auto space-y-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-shg-primary mb-2">Offline Verification</p>
          <h2 className="text-2xl font-black font-headline text-on-surface">Launch Scanner</h2>
          <p className="text-sm text-muted-foreground mt-2">Verify QR payloads and transaction hashes for banks and NGO field officers.</p>
        </div>
        <div className="bg-white border border-border/50 rounded-2xl p-6 space-y-4">
          ...
          <textarea
            ...
            className="w-full border border-border rounded-lg p-3 text-sm min-h-28"
    ...
    return (
      <div className="p-6 lg:p-10 max-w-5xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2">Approve new grant tranches and monitor grant-linked treasury inflows.</p>
        </div>
        <div className="bg-white border border-border/50 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase text-muted-foreground">Current Regional Liquidity</p>
            <p className="text-2xl font-black font-headline text-shg-secondary mt-1">{formatRegionalLiquidity(stats?.regionalLiquidity || 0)}</p>
          </div>
          <button
        ...
        <div className="bg-white border border-border/50 rounded-2xl p-6">
          <h3 className="text-lg font-bold font-headline mb-4">Grant-Related Ledger Activity</h3>
          <div className="space-y-3">
            {(ledger || []).filter((entry: any) => entry.amount > 0).slice(0, 8).map((entry: any) => (
            ...
            {(ledger || []).filter((entry: any) => entry.amount > 0).length === 0 && (
              <p className="text-sm text-muted-foreground">No grant-linked entries yet. Approve a grant to seed this feed.</p>
            )}
          </div>
        </div>
      </div>
  ...
  if (activeSection === 'audit') {
    return (
      <div className="p-6 lg:p-10 max-w-6xl mx-auto space-y-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-shg-primary mb-2">Institution Audit</p>
          <h2 className="text-2xl font-black font-headline text-on-surface">Ledger Logs</h2>
          <p className="text-sm text-muted-foreground mt-2">Live immutable transaction events across member, leader, and grant operations.</p>
        </div>
        <div className="bg-white border border-border/50 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold font-headline">Real-Time Ledger Stream</h3>
            <button
              onClick={() => toast.success('Ledger is synced with backend API')}
          ...
          <div className="space-y-3">
            {(ledger || []).length === 0 ? (
              <p className="text-sm text-muted-foreground">No ledger logs available yet. Generate transactions to populate this feed.</p>
            ) : (
            ...
            <div className="bg-white px-6 py-4 rounded-xl border border-border/50">
              ...
              {loadingStats ? <Skeleton className="h-7 w-24" /> : (
                <span className="text-2xl font-black font-headline text-shg-secondary">
                  {formatRegionalLiquidity(stats?.regionalLiquidity || 0)}
                </span>
              )}
            </div>
            <div className="bg-white px-6 py-4 rounded-xl border border-border/50">
