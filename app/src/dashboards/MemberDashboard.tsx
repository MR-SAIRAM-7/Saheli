...
export default function MemberDashboard({ activeSection = 'passport', onOpenAIAssistant }: MemberDashboardProps) {
  ...
  const member = React.useMemo(() => {
    ...
    return {
      ...
      trustScore: passport?.metadata?.score ?? rawMember.trustScore ?? 500,
      trustGrade: passport?.metadata?.grade ?? (rawMember.trustGrade || 'New Member'),
      ...
      badges: passport?.metadata?.badges || rawMember.badges || ['New Member'],
    };
  }, [rawMember, passport]);
  ...
  const handleMintPassport = async () => {
    ...
    try {
      ...
      await Promise.all([refetchPassport(), refetchMember()]);
      toast.success('Financial passport minted and anchored on-chain.');
    } catch (err: any) {
      toast.error(err?.message || 'Unable to mint passport');
    } finally {
  ...
  const handleRefreshPassport = async () => {
    ...
    try {
      ...
      await Promise.all([refetchPassport(), refetchMember()]);
      toast.success('Passport metadata refreshed from latest transactions.');
    } catch (err: any) {
      toast.error(err?.message || 'Unable to refresh passport');
    } finally {
      setPassportBusy(false);
    }
  };
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="flex-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-3 p-3 rounded-xl bg-white/10 border border-white/20 max-w-sm">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[10px] uppercase tracking-wider font-bold text-white/70 flex items-center gap-1">
                        <Shield className="w-3 h-3" />
                        d-SBT Passport
                      </p>
                      {passport?.sbtAssetId ? (
                        <span className="text-[10px] font-bold text-emerald-300">Asset #{passport.sbtAssetId}</span>
                      ) : (
                        <span className="text-[10px] font-bold text-amber-200">Not Minted</span>
                      )}
                    </div>
                    <p className="text-[11px] text-white/70 mt-1">
                      {passport?.metadata
                        ? `Version ${passport.metadata.version} · Updated ${new Date(passport.metadata.lastUpdatedAt).toLocaleString('en-IN')}`
                        : 'Mint your soulbound financial passport to start immutable credit history.'}
                    </p>
                    <div className="mt-2 flex gap-2">
                      {!passport?.sbtAssetId ? (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-white text-shg-primary text-xs font-bold disabled:opacity-60"
                        >
                          {passportBusy ? 'Minting...' : 'Mint Passport'}
                        </button>
                      ) : (
                        <button
                          onClick={handleRefreshPassport}
                          disabled={passportBusy}
                          className="px-3 py-1.5 rounded-lg bg-white text-shg-primary text-xs font-bold inline-flex items-center gap-1 disabled:opacity-60"
                        >
                          <RefreshCw className={`w-3 h-3 ${passportBusy ? 'animate-spin' : ''}`} />
                          Refresh Metadata
                        </button>
                      )}
                    </div>
