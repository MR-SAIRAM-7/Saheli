...
function mapLoan(doc: any) {
  return {
    ...
    algorandExplorerUrl: doc.algorandExplorerUrl,
    algorandNetwork: doc.algorandNetwork,
  };
}
...
router.get('/bank-queue/list', protect, authorize('bank', 'leader'), async (_req: Request, res: Response) => {
...
router.post('/bank-queue/:id/process', protect, authorize('bank'), async (req: Request, res: Response) => {
...
router.get('/:id', protect, async (req: AuthRequest, res: Response) => {
  ...
  if (req.user.role === 'member' && String((loan as any).user?._id || (loan as any).user) !== String(req.user._id)) {
    res.status(403).json({ success: false, error: 'Not authorized to view this loan' });
    return;
  }
...
router.post('/request', protect, authorize('member'), async (req: AuthRequest, res: Response) => {
  ...
  if (String(req.user._id) !== String(memberId)) {
    res.status(403).json({ success: false, error: 'You can only request loans for your own account' });
    return;
  }
...
router.post('/:id/approve', protect, authorize('leader'), async (req: Request, res: Response) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    res.status(404).json({ success: false, error: 'Loan not found' });
    return;
  }
  ...
  if (action) {
    ...
    if (loan.status !== 'pending') {
      ...
      action.transactionId = loan.transactionId || undefined;
    }
    await action.save();
  }
