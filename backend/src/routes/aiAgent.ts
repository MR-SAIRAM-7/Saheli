...
async function saveMemberTransaction(params: {
  ...
  const member = await User.findById(params.userId).select('name walletAddress');
  const chain = params.transactionId
    ? {
    ...
    : await executeOnChainRecord({
        ...
        forceValueTransfer: params.type === 'loan_disbursement' || params.type === 'withdrawal',
      });
  ...
  await Transaction.create({
    ...
    metadata: {
      source: 'ai.agent',
      usedExistingTxId: !!params.transactionId,
    },
  });

  return chain.transactionId;
}
  ...
  switch (intent.intent) {
    case 'deposit': {
      ...
      if (member) {
        ...
        await member.save();
        await refreshPassport(String(member._id), 'ai_deposit');
      }
    ...
    case 'withdraw': {
      ...
      if (member) {
        ...
        await member.save();
        await refreshPassport(String(member._id), 'ai_withdrawal');
      }
    ...
    case 'loan': {
      ...
      if (member) {
        if (agentResult.autoApproved && agentResult.transactionId) {
          ...
          await member.save();
          await refreshPassport(String(member._id), 'ai_loan_disbursed');
    ...
    case 'qr': {
      const transactionId = member
        ? await saveMemberTransaction({
            userId: String(member._id),
            type: 'deposit',
            amount: 0,
            description: 'QR proof anchor generated by AI assistant',
          })
        : undefined;
      return {
...
router.get('/log', protect, async (_req: Request, res: Response) => {
...
router.get('/insights', protect, async (_req: Request, res: Response) => {
